...
from util import timeit, orient_pin_transform
...
class PlaceEnv(gym.Env):
    ...
    def CANVAS_SLICE(self) -> slice:
        return slice(1, 1 + self.grid * self.grid)
    
    @cached_property
    def WIRE_SLICE(self) -> slice:
        return slice(1 + self.grid * self.grid, 1 + self.grid * self.grid * 2)
    
    @cached_property
    def POS_SLICE(self) -> slice:
        return slice(1+self.grid*self.grid*2, 1+self.grid*self.grid*3)
    ...
    def get_net_img(self, orient:int=0, is_next_next = False):
        """
            WireMask
        """
        net_img = np.zeros((self.grid, self.grid))
        ...
        for net_name in self.placedb.node2net_dict[next_node_name]:
            if net_name in self.net_min_max_ord:
                ...
                size = (self.placedb.node_info[next_node_name]['width'], self.placedb.node_info[next_node_name]['height'])
                delta_pin_x = self.placedb.net_info[net_name]["nodes"][next_node_name]["pin_offset"][0]
                delta_pin_y = self.placedb.net_info[net_name]["nodes"][next_node_name]["pin_offset"][1]
                delta_pin_x, delta_pin_y = orient_pin_transform(orient, size, (delta_pin_x, delta_pin_y))
                delta_pin_x = round(delta_pin_x/self.ratio)
                delta_pin_y = round(delta_pin_y/self.ratio)
                ...
                for i in range(end_x, self.grid):
                    net_img[i, :] += (i - end_x + base_offset_x) * weight
                for j in range(end_y, self.grid):
                    net_img[:, j] += (j - end_y + base_offset_y) * weight
        return net_img

    def step(self, action:int, orient: int = 0):
        ...
        # 测试摆放方向时的对照组，将宏的长和宽对调
        if orient % 2 == 1:
            size_x, size_y = size_y, size_x

        assert abs(size_x - self.state[-2]*self.grid) < 1e-5 or abs(size_x - self.state[-1]*self.grid) < 1e-5
        assert abs(size_y - self.state[-2]*self.grid) < 1e-5 or abs(size_y - self.state[-1]*self.grid) < 1e-5
        ...
        for net_name in self.placedb.node2net_dict[node_name]:
            ...
            size = (self.placedb.node_info[node_name]['width'], self.placedb.node_info[node_name]['height'])
            pin_offset_x, pin_offset_y = orient_pin_transform(orient, size, (pin_offset_x, pin_offset_y))
        ...
        if self.num_macro_placed < self.placed_num_macro:
            net_img = self.get_net_img(orient)
        ...
        if not done: # get next macro size and pre-mask the solution
            ...
            if self.num_macro_placed + 1 < self.placed_num_macro:
                ...
                # 测试摆放方向时的对照组，将宏的长和宽对调
                if orient % 2 == 1:
                    next_x_2, next_y_2 = next_y_2, next_x_2
                mask_2 = self.get_mask(next_x_2, next_y_2)
        else:
            next_x = 0
            next_y = 0
