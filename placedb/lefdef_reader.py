...
class LefDefReader(DesignReader):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def_path, lef_list = self._parse_lefdef_files(design_path)

        assert def_path is not None, f"No def file found in {design_path}"
        assert len(lef_list) > 0, f"No lef file found in {design_path}"
            ...
            for idx, line in enumerate(read_file):
                ...
                if READ_MACRO:
                    if line.lstrip().startswith("SIZE"):
                        ...
                        print(
                            f"[warning] discard line in macro/pin '{cell_name}/{pin_name}': '[{idx+1}]{line.rstrip()}'",
                            file=self.error_file,
                        )
                else:
                    print(
                        f"[warning] discard line: '[{idx+1}]{line.rstrip()}'",
                        file=self.error_file,
                    )
        ...
        for line in read_file:
            ...
            if READ_COMPONENTS:
                if "FIXED" in line:
                    ...
                    if name not in self.def_instance_dict.keys():
                        self.def_instance_dict[name] = {}

                    self.def_instance_dict[name]["type"] = type
                    self.def_instance_dict[name]["coordinate"] = coordinate
                    self.def_instance_dict[name]["size"] = size
                    self.def_instance_dict[name]["orientation"] = orientation
                    self.def_instance_dict[name]["attributes"] = attributes
            ...
            if READ_PINS:
                ...
                if line.startswith("-"):
                    ...
                    if name not in self.def_pin_dict.keys():
                        self.def_pin_dict[name] = {}
                    self.def_pin_dict[name]["direction"] = direction
                    ...
                    if name not in self.def_pin_dict.keys():
                        self.def_pin_dict[name] = {}
                    self.def_pin_dict[name]["layer_size"] = layer_size
                    ...
                    if name not in self.def_pin_dict.keys():
                        self.def_pin_dict[name] = {}
                    self.def_pin_dict[name]["attributes"] = attributes
                    self.def_pin_dict[name]["coordinate"] = coordinate
                    self.def_pin_dict[name]["orientation"] = orientation

            if READ_NETS:  # get route_net_dict
                if line.startswith("-"):
                    net = line.split()[1]
                    if net not in self.def_net_dict.keys():
                        net_id = len(self.def_net_dict)
                        self.def_net_dict[net] = {
                            "id": net_id,
                            "key": net,
                            "nodes": [],
                        }
                    ...
                    for k in l:
                        if k == "(":
                            self.def_net_dict[net]["nodes"].append([l[n + 1], l[n + 2]])
                        n += 1
                else:
                    if (
                        len(line.strip()) < 3
                        or line.strip().startswith("NETS")
    ...
    def _parse_row(self, line: str):
        ...
        self.def_row_list.append(row_item)
    ...
    def place_net_dict(self) -> dict:
        net_dict = {}
        for net_name in sorted(self.def_net_dict):
            net_dict[net_name] = {
                "id": len(net_dict),
                "key": net_name,
                "weight": 1.0,
            ...
            for node_name, pin_name in self.def_net_dict[net_name]["nodes"]:
                if node_name == "PIN":
                    net_dict[net_name]["ports"].append(pin_name)
                else:
    ...
    def place_node_dict(self) -> dict:
        node_dict = {}
        for node_name in self.def_instance_dict:
            node_dict[node_name] = {
                "coordinate": None,
                "orientation": None,
                "attributes": self.def_instance_dict[node_name]["attributes"],
                "node_type": self.def_instance_dict[node_name]["type"],
            }
        return node_dict
    ...
    def place_port_dict(self) -> dict:
        port_dict = {}
        for port_name in self.def_pin_dict:
            port_dict[port_name] = {
                "coordinate": self.def_pin_dict[port_name]["coordinate"],
                "direction": self.def_pin_dict[port_name]["direction"],
            }
        return port_dict
    ...
    def node_phisical_info(self) -> dict:
        phisical_dict = {}
        for node_type in self.lef_dict:
            phisical_dict[node_type] = {
                "size": self.lef_dict[node_type]["size"],
                "pins": {}
            }
            for pin_name, pin_info in self.lef_dict[node_type]["pin"].items():
                pin_offset_x = (pin_info["rect_left"] + pin_info["rect_right"]) / 2
                pin_offset_y = (pin_info["rect_lower"] + pin_info["rect_upper"]) / 2
                phisical_dict[node_type]["pins"][pin_name] = {
                    "pin_offset": (pin_offset_x, pin_offset_y),
                    "direction": pin_info["direction"],
                }

        return phisical_dict